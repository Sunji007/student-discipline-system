            ...
            <form method="POST" action="{{ route('admin.teachers.update', $teacher->TeacherID) }}">
                ...
                <div class="form-row">
                    @php
                        ...
                        $selectedPrefix = '';
                        $firstNameOnly = $userFirstName;
                        foreach ($prefixes as $p) {
                            if (str_starts_with($userFirstName, $p)) {
                                $selectedPrefix = $p;
                                $firstNameOnly = substr($userFirstName, strlen($p));
                                break;
                            }
                        }
                    @endphp
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            ...
                            <input type="text" name="FirstName" id="FirstName" class="form-control @error('FirstName') is-invalid @enderror" value="{{ old('FirstName', $firstNameOnly) }}" required placeholder="ระบุชื่อจริง..." style="flex: 1;">
                        </div>
                        @error('FirstName')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="LastName" id="LastName" class="form-control @error('LastName') is-invalid @enderror" value="{{ old('LastName', $teacher->user->LastName ?? '') }}" required placeholder="ระบุนามสกุล...">
                        @error('LastName')
                ...
                <div class="form-group" style="margin-bottom: 1rem;">
                    ...
                    <select name="department_id" id="Department" class="form-control">
                        ...
                        @foreach($departments as $dept)
                            <option value="{{ $dept->department_id }}" {{ old('department_id', $teacher->department_id) == $dept->department_id ? 'selected' : '' }}>{{ $dept->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" style="margin-bottom: 1rem;">
                    ...
                    <div id="advisory-rooms-container" style="display: flex; flex-direction: column; gap: 0.5rem;">
                        @php
                            $rooms = old('advisory_rooms', $teacher->advisory_rooms);
                        @endphp
                        @if(empty($rooms))
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="CitizenID" id="CitizenID" class="form-control @error('CitizenID') is-invalid @enderror" value="{{ old('CitizenID', $teacher->user?->CitizenID) }}" required placeholder="เช่น 1234567890123" maxlength="13" readonly style="background-color: #f1f5f9; cursor: not-allowed;">
                        @error('CitizenID')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
