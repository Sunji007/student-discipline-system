...
<head>
    ...
    <title>เปลี่ยนรหัสผ่านใหม่ — โรงเรียนศิริราษฎร์สามัคคี</title>
...
<body>
    ...
    <div class="login-card">
        ...
        <div class="card-body">
            <h2 class="login-title">เปลี่ยนรหัสผ่านใหม่</h2>
            <p class="login-subtitle">กรุณากรอกรหัสผ่านใหม่ของท่าน</p>
            ...
            <form method="POST" action="{{ route('password.update') }}">
                ...
                <div class="form-group">
                    ...
                    <div id="pw-rules-box" style="font-size: 0.78rem; color: var(--text-sub); margin-top: 0.5rem; line-height: 1.5; background: var(--primary-pale); padding: 0.6rem 0.8rem; border-radius: 8px; border: 1px solid var(--border); display: none; transition: all 0.25s ease-in-out;">
                        <span style="font-weight: 700; color: var(--primary);">🛡️ เงื่อนไขรหัสผ่านใหม่:</span>
                        <ul style="margin: 0.35rem 0 0 0.25rem; padding: 0; list-style: none; display: flex; flex-direction: column; gap: 0.25rem;">
                            <li id="rule-len" style="display: flex; align-items: center; gap: 0.4rem; color: #64748b; transition: color 0.2s;">
                                <span class="icon">⚪</span> ยาวอย่างน้อย <strong>8 ตัวอักษร</strong> ขึ้นไป
                            </li>
                            <li id="rule-upper" style="display: flex; align-items: center; gap: 0.4rem; color: #64748b; transition: color 0.2s;">
                                <span class="icon">⚪</span> มีตัวอักษรภาษาอังกฤษตัวพิมพ์ใหญ่ (A-Z) อย่างน้อย <strong>1 ตัว</strong>
                            </li>
                            <li id="rule-lower" style="display: flex; align-items: center; gap: 0.4rem; color: #64748b; transition: color 0.2s;">
                                <span class="icon">⚪</span> มีตัวอักษรภาษาอังกฤษตัวพิมพ์เล็ก (a-z) อย่างน้อย <strong>1 ตัว</strong>
                            </li>
                            <li id="rule-num" style="display: flex; align-items: center; gap: 0.4rem; color: #64748b; transition: color 0.2s;">
                                <span class="icon">⚪</span> มีตัวเลข (0-9) อย่างน้อย <strong>1 ตัว</strong>
                            </li>
                            <li id="rule-symbol" style="display: flex; align-items: center; gap: 0.4rem; color: #64748b; transition: color 0.2s;">
                                <span class="icon">⚪</span> มีอักขระพิเศษ (เช่น @, #, $, !) อย่างน้อย <strong>1 ตัว</strong>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="input-wrapper">
                        ...
                        <div class="password-wrapper">
                            <input
                                ...
                                required
                                placeholder="ยืนยันรหัสผ่านใหม่ (ให้ตรงกับช่องด้านบน)"
                                style="padding-left: 2.6rem;"
                                oninput="this.value = this.value.replace(/[\u0e00-\u0e7f]/g, '')"
                            >
                ...
                <div style="text-align: center; margin-top: 1rem;">
                    <a href="{{ route('login') }}" style="color: var(--text-muted); font-size: 0.85rem; text-decoration: none; font-weight: 600; transition: color var(--dur) var(--ease-std);" 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var rulesBox = document.getElementById('pw-rules-box');

        if (!pwInput || !rulesBox) return;

        function updateRule(elementId, isValid, isNotEmpty) {
            ...
            if (isValid) {
                ...
                el.style.color = '#e11d48'; // red
                el.style.fontWeight = 'normal';
            } else {
                iconSpan.textContent = '⚪'; // empty
                el.style.color = '#64748b'; // gray
                el.style.fontWeight = 'normal';
            }
        }
        ...
        function checkPassword() {
            ...
            updateRule('rule-len', val.length >= 8, isNotEmpty);
            updateRule('rule-upper', /[A-Z]/.test(val), isNotEmpty);
            updateRule('rule-lower', /[a-z]/.test(val), isNotEmpty);
            updateRule('rule-num', /\d/.test(val), isNotEmpty);
            updateRule('rule-symbol', /[^a-zA-Z0-9\sก-๙]/.test(val), isNotEmpty);
        }
