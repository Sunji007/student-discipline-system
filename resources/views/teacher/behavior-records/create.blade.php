        ...
        <div class="card-body-pad">
            <form method="POST" action="{{ route('teacher.behavior-records.store') }}" enctype="multipart/form-data" id="behaviorRecordForm">
                ...
                <div class="form-group" style="margin-bottom: 1.25rem;">
                    <label class="form-label">ค้นหากฎเกณฑ์พฤติกรรม</label>
                    <input type="text" id="ruleSearchInput" class="form-control" style="background:#f8fafc; border-color:#cbd5e1; margin-bottom:0.5rem; padding: 0.75rem 1rem; line-height: 1.5;" placeholder="พิมพ์เพื่อค้นหากฎเกณฑ์...">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterRules(selectedVal = null) {
        ...
        if (descriptionLabel) {
            descriptionLabel.textContent = isPositive ? 'รายละเอียดการทำความดี/กิจกรรม' : 'รายละเอียด/พฤติการณ์เพิ่มเติม';
        }

        const descriptionTextarea = document.getElementById('descriptionTextarea');
        if (descriptionTextarea) {
            descriptionTextarea.placeholder = isPositive ? 'อธิบายรายละเอียดการทำความดี หรือกิจกรรมจิตอาสา...' : 'ระบุเหตุการณ์เพิ่มเติม (ถ้ามี)';
        }
