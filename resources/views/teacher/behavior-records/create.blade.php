...
<div style="max-width:680px;">
    <div class="card">
        ...
        <div class="card-body-pad">
            <form method="POST" action="{{ route('teacher.behavior-records.store') }}">
                @csrf
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="RecordDate" class="form-control {{ $errors->has('RecordDate') ? 'is-invalid' : '' }}"
                               value="{{ old('RecordDate', date('Y-m-d')) }}" required>
                ...
                <div class="form-group">
                    <label class="form-label">พฤติกรรม/กฎเกณฑ์ <span style="color:var(--red)">*</span></label>
                    <select name="RuleID" class="form-control {{ $errors->has('RuleID') ? 'is-invalid' : '' }}" id="ruleSelect" required>
                        ...
                        @foreach($rules as $rule)
                            <option value="{{ $rule->RuleID }}" 
                                ...
                                {{ $rule->RuleName }} ({{ $rule->RuleType === 'ตัดคะแนน' ? '-' : '+' }}{{ abs($rule->ScoreModifier) }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label class="form-label">รายละเอียด/พฤติการณ์เพิ่มเติม</label>
                    <textarea name="Description" class="form-control" rows="3" placeholder="ระบุเหตุการณ์เพิ่มเติม (ถ้ามี)">{{ old('Description') }}</textarea>
                </div>
                ...
                <div style="display:flex; gap:0.75rem; margin-top:1.5rem;">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> ส่งบันทึก (รอฝ่ายปกครองอนุมัติ)
                    </button>
...
<script>
    ...
    function filterRules(selectedVal = null) {
        ...
        ruleSelect.innerHTML = '<option value="">เลือกพฤติกรรม</option>';
